...
系统会默认创建一个_id主键字段，用户不需要手动创建。

如果写入数据时设置了_id, 则将使用指定的_id。 如果未设置, 则由 Vearch 生成。

如果写入数据时指定的_id已经存在, 则更新现有数据; 否则, 它将被插入。
...
(1) filter 条件支持多个, 多个条件之间是交的关系, 即最外层operator目前只支持AND。
...
+----------+--------+----------+---------------------------------------+
| operator | string | 是       | 操作符, 支持 >, >=, <, <=, IN, NOT IN |
+----------+--------+----------+---------------------------------------+
| value    | json   | 是       | 过滤值                                |
+----------+--------+----------+---------------------------------------+
...
+-----------------+----------+----------+-------------------------------------------------------------------+
| is_brute_search | int      | 否       | 0-不使用暴力搜索，1-使用暴力搜索；默认值0                                                             |
+-----------------+----------+----------+-------------------------------------------------------------------+
| vector_value    | bool     | 否       | 默认false, 是否返回向量                                           |
+-----------------+----------+----------+-------------------------------------------------------------------+
...
(1) vector 支持多个(对应定义表结构时包含多个向量字段)。

(2) field 指定创建表时向量字段的名称。
...
- is_brute_search  0使用索引搜索(建完索引前查询结果为空),  1使用暴力搜索, 默认值0。当某些向量索引未开始训练时，可设置为1使用暴力搜索来获取结果。
...
后台接收到请求后按表结构定义的向量字段维度进行拆分, 按顺序返回匹配结果。
...
表空间定义时支持多个向量字段, 因此查询时可以支持相应数据的特征进行查询。以每条记录两个向量为例: 定义表结构字段
::
...
filters结构说明: 参考query接口部分对filters的说明
